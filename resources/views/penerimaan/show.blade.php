    ...
    <div class="bg-gradient-to-br from-indigo-50 to-blue-50 border-2 border-indigo-200 rounded-xl p-6 shadow-lg">
        <div class="flex items-center justify-between mb-4">
            <div>
                ...
                <p class="text-sm text-indigo-600 mt-1">ID Penerimaan: #{{ $header->idpenerimaan }}</p>
            </div>
            <div class="text-right">
                ...
                <span class="inline-block px-4 py-2 rounded-lg font-semibold text-sm mt-1
                    ...
                    {{ $progress >= 100 ? '✓ Selesai' : '⏳ Dalam Proses' }}
                </span>
            </div>
        </div>
        ...
        <div class="grid grid-cols-3 gap-6 mb-4">
            ...
            <div class="bg-white/70 backdrop-blur rounded-lg p-4">
                ...
                <p class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($header->created_at)->format('d M Y, H:i') }}</p>
            </div>
        </div>
...
{{-- ✅ Progress Bar dengan Animasi --}}
        <div class="bg-white/90 backdrop-blur rounded-lg p-4">
            <div class="flex justify-between items-center mb-3">
                ...
                <span class="text-2xl font-bold {{ $progress >= 100 ? 'text-green-600' : 'text-yellow-600' }}">
                    {{ $progress }}%
                </span>
            </div>
            <div class="w-full bg-gray-200 rounded-full h-4 overflow-hidden shadow-inner">
                <div class="{{ $progress >= 100 ? 'bg-gradient-to-r from-green-400 to-green-600' : 'bg-gradient-to-r from-yellow-400 to-yellow-600' }} 
...
{{-- ✅ DAFTAR DETAIL PENERIMAAN (CARD PER DETAIL) --}}
    <div class="space-y-4">
        ...
        @php
            // Group detail berdasarkan timestamp untuk identifikasi batch penerimaan
            $groupedDetails = $detail->groupBy(function($item) {
                return \Carbon\Carbon::parse($item->created_at ?? now())->format('Y-m-d H:i:s');
            });
            $detailNumber = 1;
        @endphp

        @foreach($groupedDetails as $timestamp => $items)
        <div class="bg-white border-2 border-gray-200 rounded-xl shadow-md hover:shadow-xl transition-all duration-300">
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            ...
                            <tr class="border-b hover:bg-indigo-50 transition">
                                ...
                                <td class="p-3 font-medium text-gray-800">{{ $d->nama_barang ?? '-' }}</td>
                                <td class="p-3 text-center">
                                    <span class="bg-green-100 text-green-800 px-3 py-1 rounded-full font-semibold text-xs">
                                        {{ number_format($d->jumlah_terima ?? 0, 0, ',', '.') }} unit
                                    </span>
                                </td>
            ...
            <table class="w-full text-sm border-2 border-red-200 rounded-lg overflow-hidden">
                ...
                <tbody class="bg-white">
                    ...
                    <tr class="border-b border-red-100 hover:bg-red-50 transition">
                        ...
                        <td class="p-3 font-medium text-gray-900">{{ $item->nama_barang }}</td>
                        <td class="p-3 text-center text-gray-700">
